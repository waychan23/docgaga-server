...
function doCopyres(done){
	...
	gulp.src(['src/*.json', 'src/*.html']).pipe(gulp.dest('build/')).on('end', counter());
	gulp.src(['src/html/**/*']).pipe(gulp.dest('build/html/')).on('end', counter());
	gulp.src(['src/css/**/*.css']).pipe(gulp.dest('build/css/')).on('end', counter());
	gulp.src(['src/img/**/*']).pipe(gulp.dest('build/img/')).on('end', counter());
}
...
gulp.task('watch-sources', ['build'], () => {
	...
	gulp.watch([bundleConfigLocation], () => {
		doBundleExternals();
		doBundleSources();
	});
});
